...
module.exports = {
    ...
    async execute(ia) {
        ...
        for (let i = 1; i <= 6; i++)
            if (
                !!(mem = ia.options.getMember(`o${i}`)) &&
                !mem.roles.cache.some((role) => role.id == tlg.modRoleID || role.id == tlg.adminRoleID) &&
                !mem.user.bot
            ) {
                await Promise.all([
        ...
        ia.embed
            ...
            .addFields([
                { name: "Observers added", value: addedList != "|" ? addedList : "None" },
            ]);
