...
module.exports = {
  ...
  async execute(ia) {
    ...
    try {
      Promise.all([rpCh.setParent(archiveCat), dcCh.setPosition(newPos), dcCh.delete(), guild.roles.resolve(camp.role).delete()]);
    } catch (error) {
      console.error(error);
    ...
    if (!campList.filter((c) => c.DM == camp.DM).length) {
      ...
      if (!dm.roles.cache.some((r) => r.position > campRoleMinPos && r.position < campRoleMaxPos)) dm.roles.add(noCampRoleID);
    }
